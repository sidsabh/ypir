...
__global__ void compute_hint_0_kernel(
    ...
    for (uint32_t outer_row = 0; outer_row < num_chunks; outer_row++) {
        ...
        // Forward NTT for each CRT modulus (all threads cooperatively)
        for (uint32_t crt = 0; crt < crt_count; crt++) {
            ntt_forward_kernel_parallel(pt_ntt + crt * poly_len, ntt_params, crt, tid, blockDim.x);
        }
        ...
        if (should_process) {
            ...
            // Inverse NTT (all threads cooperatively)
            for (uint32_t crt = 0; crt < crt_count; crt++) {
                ntt_inverse_kernel_parallel(tmp_col + crt * poly_len, ntt_params, crt, tid, blockDim.x);
            }
